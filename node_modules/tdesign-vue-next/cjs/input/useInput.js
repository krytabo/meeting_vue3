...
function useInput(props, expose) {
  ...
  var inputValueChangeHandle = function inputValueChangeHandle(e) {
    ...
    if (props.maxcharacter && props.maxcharacter > 0) {
      var stringInfo = utils_helper.getCharacterLength(val, props.maxcharacter);
      if (_typeof__default["default"](stringInfo) === "object") {
        if (!props.allowInputOverMax) {
          val = stringInfo.characters;
        }
        limitNumber.value = "".concat(stringInfo.length, "/").concat(props.maxcharacter);
      }
    }
    if (props.maxlength && props.maxlength > 0) {
      ...
      limitNumber.value = "".concat(val.length, "/").concat(props.maxlength);
    }
    setInnerValue(val, {
      e: e
    });
  ...
  vue.watch(innerStatus, function () {
  ...
  var updateLimitNumber = function updateLimitNumber() {
    if (props.maxcharacter && props.maxcharacter > 0) {
      var len = utils_helper.getCharacterLength(String(innerValue.value));
      limitNumber.value = "".concat(len, "/").concat(props.maxcharacter);
    }
    if (props.maxlength && props.maxlength > 0) {
      var val = !innerValue.value ? "" : innerValue.value;
      limitNumber.value = "".concat(String(val).length, "/").concat(props.maxlength);
    }
  };
